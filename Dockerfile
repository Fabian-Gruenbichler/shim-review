...
#RUN echo "deb [check-valid-until=no] https://snapshot.debian.org/archive/debian/20250604T000000Z/ unstable main" > /etc/apt/sources.list
#RUN echo "deb-src [check-valid-until=no] https://snapshot.debian.org/archive/debian/20250604T000000Z/ unstable main" >> /etc/apt/sources.list
##
#########
...
WORKDIR /shim-review
RUN git checkout proxmox-shim-16.0-amd64-20250604
WORKDIR /
...
RUN ls -lha
RUN git checkout proxmox/16.0-2+pmx1
