...
*******************************************************************************
https://git.proxmox.com/?p=efi-boot-shim.git;a=shortlog;h=refs/tags/proxmox/16.0-2%2Bpmx1
...
While this is no longer broken with Trixie, the patch is still correct and we want to
minimize the delta between our releases.
*******************************************************************************
...
*******************************************************************************
Yes. Our kernels are currently based on 6.14.
...
*******************************************************************************
shim_16.0-2+pmx1_amd64.build (build log)
shim_16.0-2+pmx1_amd64.buildinfo (versions of packages installed in the build environment)
...
*******************************************************************************
We've added a kernel series based on 6.14, a new product based on the shared
Secure Boot implementation, updated our base distro Debian Trixie including
Grub2 2.12.

We also plan to start signing systemd-boot during this release cycle.

*******************************************************************************
### What is the SHA256 hash of your final shim binary?
*******************************************************************************
`24e97d2bffc9d48cc0e3b74a114c7e8edda33ad71ba1d836e275eac00af3682c`
...
grub.debian,5,Debian,grub2,2.12-7+pmx1,https://tracker.debian.org/pkg/grub2
grub.debian13,1,Debian,grub2,2.12-7+pmx1,https://tracker.debian.org/pkg/grub2
grub.peimage,2,Canonical,grub2,2.12-7+pmx1,https://salsa.debian.org/grub-team/grub/-/blob/master/debian/patches/secure-boot/efi-use-peimage-shim.patch
grub.proxmox,1,Proxmox,grub2,2.12-7+pmx1,https://git.proxmox.com/?p=grub2.git
```
...
we haven't yet prepared systemd-boot packaging, but the SBAT entries there
would mimic Debian's with an additional one namespaced to .proxmox
...
*******************************************************************************
Including modules 
	all_video boot btrfs cat chain configfile echo efifwsetup efinet ext2 fat
font f2fs gettext gfxmenu gfxterm gfxterm_background gzio halt help hfsplus
iso9660 jfs jpeg keystatus loadenv loopback linux ls lsefi lsefimmap
lsefisystab lssal memdisk minicmd normal ntfs part_apple part_msdos part_gpt
password_pbkdf2 peimage png probe reboot regexp search search_fs_uuid
search_fs_file search_label serial sleep smbios squash4 test tpm true video xfs
zfs zfscrypt zfsinfo cpuid play
	 in obj/monolithic/grub-efi-amd64/gcdx64.efi
...
password_pbkdf2 peimage png probe reboot regexp search search_fs_uuid
search_fs_file search_label serial sleep smbios squash4 test tpm true video xfs
zfs zfscrypt zfsinfo cpuid play cryptodisk gcry_arcfour gcry_blowfish
gcry_camellia gcry_cast5 gcry_crc gcry_des gcry_dsa gcry_idea gcry_md4 gcry_md5
gcry_rfc2268 gcry_rijndael gcry_rmd160 gcry_rsa gcry_seed gcry_serpent
gcry_sha1 gcry_sha256 gcry_sha512 gcry_tiger gcry_twofish gcry_whirlpool luks
luks2 lvm mdraid09 mdraid1x raid5rec raid6rec
	 in obj/monolithic/grub-efi-amd64/grubx64.efi
...
password_pbkdf2 peimage png probe reboot regexp search search_fs_uuid
search_fs_file search_label serial sleep smbios squash4 test tpm true video xfs
zfs zfscrypt zfsinfo cpuid play http tftp
	 in obj/monolithic/grub-efi-amd64/grubnetx64.efi
...
search_fs_file search_label serial sleep smbios squash4 test tpm true video xfs
zfs zfscrypt zfsinfo cpuid play http tftp
	 in obj/monolithic/grub-efi-amd64/grubnetx64-installer.efi
...
*******************************************************************************
Grub2: based on Debian Trixies's 2.12-7, packaged as 2.12-7+pmx1.

systemd-boot: will likely be based on Debian Trixie's 257.5-2
...
systemd-boot is currently not used with SecureBoot, but without, we are using
type #1 loader entries, with no plans to switch to vendor-provided UKIs, even
once signing would be enabled.
...
*******************************************************************************
Currently 6.14 as base (based on the corresponding Ubuntu kernel series).
...
on upstream 6.14.x, included via git submodule in `submodules/ubuntu-kernel`),
